...
/**
 * Start playback on the last-used zone.
 */
export default async function Command() {
  try {
    ...
    const lastZoneId = await ZonePersistenceService.getLastZone("play");
    const zone = zones.find((z) => z.zone_id === lastZoneId) || zones[0];

    await control(zone, "play");
    await ZonePersistenceService.setLastZone("play", zone.zone_id);
    await showHUD(`▶️ Playing on ${zone.display_name}`);
  } catch (error) {
    await showToast({
      style: Toast.Style.Failure,
